...
public class ScanLineFill : MonoBehaviour
{
	...
	public ColorChoose currentColorChoose = ColorChoose.Grad;
                ...
                for (int i = 0; i < ActiveList.Count; i++)
                {
                    ...
                    if (edgePair.Count >= 2)
                    {
                        ...
                        for (int itPair = pair1.x; itPair <= pair2.x; itPair++)
                        {
							...
							switch (this.currentColorChoose)
							{
								case ColorChoose.Grad:
									color = gradient.Evaluate(Mathf.InverseLerp(minX, maxX, y));
									newColor = color + previousColor * (1f - color.a);
									targetTex.SetPixel(itPair, y, newColor);
									break;
